...
#' @param structure.learning.args.list List of arguments passed to structure.learning.algorithm, in particular distance argument if
...
#' @param forbid.GG      Arcs between grid or G nodes will be forbidden.
#' @param forbid.DD       Arcs between local, i.e. station or D nodes, will be forbidden.
...
#' @param keep.dynamic.distance When \code{dynamic = TRUE} and local learning is employed, if set to TRUE it will use its corresponding
#' distance (See \code{Details}) between nodes from diferent epochs.
#' @param forbid.backwards When \code{dynamic = TRUE}, set to TRUE to forbid arcs going back in time.
#' @param forbid.past.dynamic.GD When \code{dynamic = TRUE}, set to TRUE to forbid arcs in the form G->D or D->G between different epochs.
#' @param forbid.dynamic.GG When \code{dynamic = TRUE} and \code{remove.past.G = FALSE}, set to TRUE to forbid arcs in the form G-G in the
#' past epochs.
#' @param forbid.past.DD When \code{dynamic = TRUE}, set to TRUE to forbid arcs in the form D-D in the past epochs.
...
#' @param fix.intermediate Set to TRUE to forbid the creation of new arcs in the next steps for already built DAGs. See \code{Details}.
#' \code{structure.learning.algorithm2} and \code{structure.learning.args.list2}. See \code{Details}.
#' @param structure.learning.algorithm2   Same as structure.learning.algorithm for the second step if \code{structure.learning.steps} is
#' employed. Ignored otherwise.
#' @param structure.learning.args.list2   Same as structure.learning.args.list for the second step if \code{structure.learning.steps} is
#' employed. Ignored otherwise.
#' @param structure.learning.algorithm3   Same as structure.learning.algorithm for the third step if \code{structure.learning.steps} with
#' 3 steps is employed. Ignored otherwise.
#' @param structure.learning.args.list3   Same as structure.learning.args.list for the third step if \code{structure.learning.steps} with
#' 3 steps is employed. Ignored otherwise.
#' See \code{Details}.
#' @param return.intermediate Add the intermediate DAGs to the output, as $intermediateDBN1 and $intermediateDBN2 (if any) if
...
build.downscalingBN <- function(data,
  ...
  if ( !(is.null(structure.learning.args.list$distance)) ){   # local learning
    ...
    structure.learning.args.list <- handleLocalLearning(step.data, structure.learning.args.list, dynamic, keep.dynamic.distance)
  ...
  if ( steps.left >= 1){
    ...
    whitelist <- bn$arcs
    if (fix.intermediate){
      structure.learning.args.list2 <- fixIntermediate(step.data$names.distribution, structure.learning.args.list2, whitelist)
    }
    ...
    DBN <-  build.downscalingBN(data,
                                ...
                                dynamic = dynamic, keep.dynamic.distance, epochs = epochs, remove.past.G = remove.past.G,
